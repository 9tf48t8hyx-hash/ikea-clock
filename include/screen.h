...
public:
  ...
  void drawRectangle(int x, int y, int width, int height, bool fill, int ledStatus,
                     uint8_t brightness = MAX_BRIGHTNESS);
  void drawCharacter(int x, int y, const std::vector<int> &bits, int bitCount,
                     uint8_t brightness = MAX_BRIGHTNESS);
  void drawNumbers(int x, int y, const std::vector<int> &numbers,
                   uint8_t brightness = MAX_BRIGHTNESS);
  void drawBigNumbers(int x, int y, const std::vector<int> &numbers,
                      uint8_t brightness = MAX_BRIGHTNESS);
  ...
  void scrollText(const std::string &text, int delayTime = 30,
                  uint8_t brightness = MAX_BRIGHTNESS, uint8_t fontid = 0);
  void scrollGraph(const std::vector<int> &graph = {}, int miny = 0, int maxy = 15,
                   int delayTime = 60, uint8_t brightness = MAX_BRIGHTNESS);
};
